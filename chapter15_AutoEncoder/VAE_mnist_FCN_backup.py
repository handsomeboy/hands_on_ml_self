...
with tf.name_scope("latent"):
    hidden5_mean = tf.layers.dense(hidden4, n_hidden5, kernel_initializer=initializer, activation=None, name='latent_mu')
    hidden5_sigma = tf.layers.dense(hidden4, n_hidden5, kernel_initializer=initializer, activation=None, name='latent_sigma')
    noise = tf.random_normal(tf.shape(hidden5_sigma), dtype=tf.float32)
    hidden5 = hidden5_mean + hidden5_sigma * noise

with tf.name_scope("decoder"):
    hidden6 = tf.layers.dense(hidden5, n_hidden6, kernel_initializer=initializer, activation=tf.nn.elu, name='decoder_hidden6')
...
with tf.name_scope("loss"):
    ...
    eps = 1e-10
    latent_loss = 0.5*tf.reduce_sum(tf.square(hidden5_mean) + tf.square(hidden5_sigma)-1 -tf.log(eps + tf.square(hidden5_sigma)))
...
with tf.Session() as sess:
    if mode == 'train':
        ...
        codings_rnd = np.random.normal(size=[n_digits, n_hidden5])
        outputs_val = outputs.eval(feed_dict={hidden5: codings_rnd})
